# src: https://github.com/facebookresearch/DrQA/blob/master/drqa/reader/test.py
...
def set_defaults(args):
    ...
    args.predictions = os.path.join(args.model_dir, args.model_name + '_beam_predictions.txt')
...
def validate_official(args, data_loader, model):
    ...
    with open(args.predictions, 'w') as fwww:
        for eid, translation in trans_dict.items():
            fwww.write(str([' '.join(pred) for pred in translation.pred_sents])[2:-2] + '\n')
    # with open(args.fixed, 'w') as fww:
    #     for eid, translation in trans_dict.items():
    #         fww.write(str(references[eid])[2:-2] + '\n')
