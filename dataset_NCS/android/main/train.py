# src: https://github.com/facebookresearch/DrQA/blob/master/drqa/reader/train.py
...
def set_defaults(args):
    ...
    args.predic_file = os.path.join(args.model_dir, args.model_name + suffix + '_predictions' + '.txt')
    args.buggy = os.path.join(args.model_dir, args.model_name + '_buggy' + '.txt')
    args.fixed = os.path.join(args.model_dir, args.model_name + '_fixed' + '.txt')
...
def validate_official(args, data_loader, model, global_stats, mode='dev'):
    ...
    with torch.no_grad():
        ...
        for idx, ex in enumerate(pbar):
            ...
            ex_ids = list(range(idx * batch, (idx * batch) + batch_size))
            predictions, targets, copy_info = model.predict(ex, replace_unk=True)

            # write prediction file
            if mode == 'test':
                with open(args.predic_file, 'a') as fww:
    ...
    if mode == 'test':
        ...
        logger.info('dev valid official: Epoch = %d | ' %
                    ...
                    'ml_loss = %.2f |' % ml_loss.avg +
                    'perplexity = %.2f' % perplexity.avg)
...
def eval_accuracies(hypotheses, references, copy_info, sources=None,
    ...
    if mode == 'test':
        with open(args.fixed, 'w') as fff:
            for key in references.keys():
                fff.write(str(references[key][0] if args.print_one_target else references[key])[2:-2] + '\n')
        with open(args.buggy, 'w') as ffff:
            for key in references.keys():
                ffff.write(str(sources[key])[2:-2] + '\n')

    if fw: fw.close()

    # fww = open(args.predic_file, 'w') if args.predic_file else None
    # for key in references.keys():
    #     if fww:
    #         if copy_info is not None and print_copy_info:
    #             prediction = hypotheses[key][0].split()
...
def main(args):
    ...
    for dev_src, dev_src_tag, dev_tgt, dataset_name in \
        ...
        if not args.only_test:
            exs = util.load_data(args, dev_files, max_examples=args.max_examples,
                                 dataset_name=dataset_name)
        else:
            exs = util.load_data(args,
                                 dev_files,
    ...
    # TRAIN/VALID LOOP
        ...
        stats = {'timer': Timer(), 'epoch': start_epoch, 'best_valid': 1e9, 'no_improvement': 0}
        ...
        for epoch in range(start_epoch, args.num_epochs + 1):
            ...
            if epoch % 10 == 0:
                model.save(args.model_file)
                model.save(os.path.join(args.model_dir, args.model_name + str(epoch) + '.mdl'))
                logger.info('save model each 20 epoches')
